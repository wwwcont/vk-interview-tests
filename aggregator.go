...
// несколько воркеров по шардам ключа, чтобы масштабировать CPU.
// В проде я бы либо делал минимальный фикс: сделать flushCh буфер 1 (или 0) или добавить default
// Автофлаш best-effort, ошибки не ломают поток; в проде добавил бы метрики/лог и backoff.
// Если события идут из разных TZ, окна могут поехать без UTC

// Сейчас Flush приоритетный и не гарантирует, что он включает уже поставленные события из eventCh
// Если требуется, можно сделать barrier marker в eventCh и выполнить flush после него или сам flash кидать в канал
