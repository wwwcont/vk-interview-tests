...
// Нужен unique constraint на idempotency_key и обработка conflict: при конфликте читаем существующий заказ и возвращаем
// Прод-решение упавшего апдейот статус: outbox/сага (воркер добьёт статус), ретрай апдейта или запись “external reservation id” + reconciliation job
